...
(*for the whole grid*)
  let rec define_mat (*mat*) line column dfnc =  
		match line with
			|9 -> dfnc
			|_ -> let dclause = define_cell (*mat*) line column 1 []  in define_mat (*mat*) (line+((column+1)/9)) ((column+1) mod 9) (dclause ::dfnc)
...
(*for one cell*)
  let rec unique_cell line column compt compt' dfnc = 
	...
	| 9 -> dfnc
	| _ -> match compt' with
		| 9 -> unique_cell line column (compt+1) (compt+2) ([{cellule = {i=line; j=column}; d = compt; signe = false};{cellule = {i=line; j=column}; 
									d= compt'; signe = false}]::dfnc)
		| _ -> unique_cell line column compt (compt'+1) ([{cellule = {i=line; j=column}; d = compt; signe = false};{cellule = {i=line; j=column};
									d= compt'; signe = false}]::dfnc)
 ;;
...
(*for the whole grid*)
 ...
 let rec unique_mat line column dfnc =
	match line with
		| 9 -> dfnc
		| _ ->  let cell_dfnc = unique_cell line column 1 2 [] in unique_mat (line+((column+1)/9)) ((column+1) mod 9) (List.append cell_dfnc dfnc)
 ;;
...
let rec valid iarray i' j' d' dfnc =
	...
	| _ -> match j' with 
		| 8 -> if d'=9 
			then valid iarray (i'+1) (i'+2) 1 ([{cellule = {i= iarray.(i').i; j=iarray.(i').j}; d=d'; signe = false};
							    {cellule ={i=iarray.(j').i; j= iarray.(j').j}; d=d'; signe=false}] :: dfnc) 
			else valid iarray i' j' (d'+1) ([{cellule = {i= iarray.(i').i; j=iarray.(i').j}; d=d'; signe = false};
							 {cellule ={i=iarray.(j').i; j= iarray.(j').j}; d=d'; signe=false}] :: dfnc)
			
		| _ -> match d' with
			| 9 -> valid iarray i' (j'+1) 1 ([{cellule = {i= iarray.(i').i; j=iarray.(i').j}; d=d'; signe = false};
							  {cellule ={i=iarray.(j').i; j= iarray.(j').j}; d=d'; signe=false}] :: dfnc) 
			| _ -> valid iarray i' j' (d'+1) ([{cellule = {i= iarray.(i').i; j=iarray.(i').j}; d=d'; signe = false};
							   {cellule ={i=iarray.(j').i; j= iarray.(j').j}; d=d'; signe=false}] :: dfnc)
	;;			
...
let rec recup_clause s formula =
	...
	| {cellule = {i= a; j=b}; d=d'; signe = c}:: rest -> 
		recup_clause rest (String.concat "|" [formula; (String.concat " " [Pervasives.string_of_int a;Pervasives.string_of_int b;Pervasives.string_of_int 												d';Pervasives.string_of_bool c;])])
	;;
...
let rec dimacs_clause s formula =
	...
	|{cellule={i=a;j=b};d=d';signe=c}::rest -> 
		let sign = if c=false then "-" else "" in dimacs_clause rest (String.concat " " [formula; (String.concat ""
											 [sign;Pervasives.string_of_int (81*a + 9*b + d')])])
 ;;
